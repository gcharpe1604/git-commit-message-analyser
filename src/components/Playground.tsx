...
import { MdArrowForward, MdCheck, MdClose, MdContentCopy, MdDelete, MdSave } from "react-icons/md";
...
export const Playground = () => {
  ...
  const { generateMessage, loading, hasApiKey } = useLLM();
  ...
  const generate = async () => {
    if (!user || !diff.trim() || !hasApiKey) return;
    const value = await generateMessage(diff);
    if (value) { setMessage(value); setMode("write"); }
  };
  ...
  return (
    <section className="playground-page">
      <header className="page-intro playground-intro">
        <div><p className="eyebrow">Commit workshop</p><h1>Write it. Test it. Ship it.</h1><p>Get immediate, rule-based feedback or generate a message from the actual patch.</p></div>
        <div className="mode-switch"><button className={mode === "write" ? "active" : ""} onClick={() => setMode("write")}>Write message</button><button className={mode === "diff" ? "active" : ""} onClick={() => setMode("diff")}>Generate from diff</button></div>
      </header>
      ...
      <div className="workshop-grid">
        <div className="workshop-editor">
          ...
          {mode === "write" ? <textarea value={message} onChange={(event) => setMessage(event.target.value)} placeholder="feat(scope): describe the change and its intent" autoFocus /> : <textarea value={diff} onChange={(event) => setDiff(event.target.value)} placeholder={"diff --git a/src/file.ts b/src/file.ts\n+ describe your change here"} />}
          <div className="editor-actions">
            <span>{mode === "write" ? "Analysis updates as you type" : !user ? "Sign in to generate from a git diff" : hasApiKey ? "AI provider ready - the diff is required" : "Add an API key in Settings to generate"}</span>
            {mode === "write" ? <div>{result && <button className="primary" onClick={save}><MdSave /> Save draft</button>}</div> : <button className="primary" onClick={generate} disabled={!user || !diff.trim() || !hasApiKey || loading}>{loading ? "Generating..." : "Generate from diff"}<MdArrowForward /></button>}
          </div>
        </div>
        ...
        <aside className={`workshop-result tone-${tone}`} aria-live="polite">
          {result ? <>
            ...
            {result.suggestion && <div className="workshop-suggestion"><span>Rule-based format suggestion</span><code>{result.suggestion}</code><div><button onClick={() => setMessage(result.suggestion || "")}>Use this</button><button aria-label="Copy suggestion" onClick={() => navigator.clipboard.writeText(result.suggestion || "")}><MdContentCopy /></button></div></div>}
